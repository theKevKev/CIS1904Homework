...
generate cfg weights alphabet bosToken =
  ...
  where
    go currentToken position keys vals accumulated
      ...
      | otherwise = do
          (logits, keys', vals') <- gpt cfg currentToken position weights keys vals
          -- temp (temperature) = 0.5 - this controls randomness. We divide the logits by the temperature before softmax.
          -- Higher temperatures produce more diverse output while lower temperatures make the outputs more deterministic.
          scaledLogits <- mapM (\l -> do temp <- leaf 0.5; divV l temp) logits
          probabilities <- softmax scaledLogits
          -- we use randomRIO to generate a random value between 0 and 1. This helps us sample the next token. See microgpt.py#L195-196
